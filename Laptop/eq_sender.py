...
class Eq:
	...
	def __init__(self):
		...
		self.upperDb = -20 # the volume corresponding with MAX (unitCols) on eq display
		...
		# and assigns frequencies to 16 bands, only the first 8 of which are shown
		listener_desc = 'pulsesrc ! spectrum interval=20000000 bands=16 threshold=-70 ! fakesink'
	...
	def on_message (self, bus, message):
		...
		if s and s.get_name() == "spectrum":
			bandMagnitudes = s['magnitude']
			# throw away other frequency bands
			offset = 2
			selectedBandMagnitudes = bandMagnitudes[offset:self.unitCols + offset] 
			...
			for unitCol in range(0,self.unitCols):
				bandMagnitude = selectedBandMagnitudes[unitCol]
